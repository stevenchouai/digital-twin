# Wiki Index

这是这个 playground 的最小索引。

## Concepts

- `AI 不是 answer machine，而应该是 asset-building machine`
- `好的 agent 要先读已有系统，再决定怎么做`
- `真正有价值的 digital twin 更像工作方式，不像人设`

## Existing Assets

### Published Articles

- [`2026-04-10-ai-workflow-is-the-product.md`](../Blog/Published/2026-04-10-ai-workflow-is-the-product.md)

### Summaries

- [`2026-04-12-agent-products.md`](./summaries/2026-04-12-agent-products.md)

### Agent Learnings

- [`2026-04-15-writing-rules.md`](./outputs/agent-learnings/2026-04-15-writing-rules.md)

## What Matters In This Workspace

- 先看历史文章和规则，再决定怎么写
- 不重复已经写过的判断
- raw material 和 published asset 分离
- 每次任务最好都留下 learning note
