...
`digital-twin` is easiest to trust when each claim points to a file you can inspect. This page collects the repo's public proof assets and what each one demonstrates.

## What the repo proves today

| Claim | Public artifact | What to verify |
|------|-----------------|----------------|
...
3. Inspect the linked capability file and output folders on GitHub.
...
This repo is intentionally small enough to audit manually. Current public-source snapshot, generated with `pygount --format=summary --folders-to-skip='.git,node_modules,venv,.venv,__pycache__,.cache,dist,build,.next,.tox,vendor,third_party,docs/.vitepress/dist' .` on 2026-05-04:
